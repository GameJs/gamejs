 ...
 *                of the sound
 * @param {Bool} if true, sound is played in a loop
 */
exports.Sound = function Sound(uriOrAudio, loop) {
   var cachedAudio;
   ...
   /**
    * test if playback has ended
    * @returns {Bool} true if playback has ended, false otherwise
    */   
   ...
   /**
    *set time to play audio from
    *@param {Number} seconds to start playing this audio from
    */
   this.seek = function(seconds) {
      if(seconds<0) throw new Error('Cannot seek audio to negative seconds.');
