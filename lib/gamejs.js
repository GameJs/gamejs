...
Surface.prototype.blit = function(src, dest, area, compositeOperation) {
   ...
   var contextModified = (compositeOperation !== undefined && compositeOperation !== 'source-over') || src._blitAlpha !== 1;
   if (contextModified) {
      this.context.save();
      this.context.globalCompositeOperation = compositeOperation || 'source-over';
      this.context.globalAlpha = src._blitAlpha;
   }

   // do we have to rotate?
   var sourceMatrix = src._matrix;
   if (sourceMatrix[0] === 1 && sourceMatrix[1] === 0 && sourceMatrix[2] === 0 &&
               sourceMatrix[3] === 1 && sourceMatrix[4] === 0 && sourceMatrix[5] == 0) {
      // no target coords
      if (arguments.length === 1) {
         ...
         // target coords and scaled
         } else if (dest instanceof Rect) {
            this._context.drawImage(src._canvas, dest.left, dest.top, dest.width || src._canvas.width, dest.height || src._canvas.width);
         }
      } else if (arguments.length >= 3) {
         // target coords as array; and source area as array
         if (dest instanceof Array && area instanceof Array) {
            var srcWidth = src._canvas.width - area[0];
            var srcHeight = src._canvas.height - area[1];
            var destWidth = src._canvas.width;
            var destHeight = src._canvas.height;
            this._context.drawImage(src._canvas, area[0], area[1], srcWidth, srcHeight, dest[0], dest[1], destWidth, destHeight);
         // target coords as array; and source area as rect
         } else if (dest instanceof Array && area instanceof Rect) {
            var destWidth = src._canvas.width - dest[0];
            var destHeight = src._canvas.height - dest[1];
            // as a fallback: user never wants to blit to zero width; fallback to
            // full width
            var srcWidth = area.width || src._canvas.width;
            var srcHeight = area.height || src._canvas.height;
            this._context.drawImage(src._canvas, area.left, area.top, srcWidth, srcHeight, dest[0], dest[1], destWidth, destHeight);
         } else if (dest instanceof Rect && area instanceof Array) {
            var srcWidth = src._canvas.width - area[0];
            var srcHeight = src._canvas.height - area[1];
            var destWidth = dest.width || src._canvas.width;
            var destHeight = dest.height || src._canvas.height;
            this._context.drawImage(src._canvas, area[0], area[1], srcWidth, srcHeight, dest.left, dest.top, destWidth, destHeight);
         } else if (dest instanceof Rect && area instanceof Rect) {
            var srcWidth = area.width || src._canvas.width;
            var srcHeight = area.height || src._canvas.height;
            var destWidth = dest.width || src._canvas.width;
            var destHeight = dest.height || src._canvas.height;
            this._context.drawImage(src._canvas, area.left, area.top, srcWidth, srcHeight, dest.left, dest.top, destWidth, destHeight);
         }
