 ...
 * @param {Number} width of the line, defaults to 1
 ###
exports.line = (surface, color, startPos, endPos, width) ->
  ctx = surface.context
  ctx.save()
  ctx.beginPath()
 ...
 * @param {Number} width width of the lines, defaults to 1
 ###
exports.lines = (surface, color, closed, pointlist, width) ->
  closed = closed || false
  ctx = surface.context
  ctx.save()
  ctx.beginPath()
 ...
 * @param {Number} width width of the circle, if not given or 0 the circle is filled
 ###
exports.circle = (surface, color, pos, radius, width) ->
  if (!radius)
 ...
 * @param {Number} width the width of line drawing the Rect, if 0 or not given the Rect is filled.
 ###
exports.rect = (surface, color, rect, width) ->
  ctx = surface.context
  ctx.save()
  ctx.beginPath()
  ctx.strokeStyle = ctx.fillStyle = color
 ...
 * @param {Number} width the width of line, if 0 or not given the arc is filled.
 ###
exports.arc = (surface, color, rect, startAngle, stopAngle, width) ->
  ctx = surface.context
  ctx.save()
  ctx.beginPath()
  ctx.strokeStyle = ctx.fillStyle = color
 ...
 * @param {Number} width the width of line, if 0 or not given the polygon is filled.
 ###
exports.polygon = (surface, color, pointlist, width) ->
  ctx = surface.context
  ctx.save()
